...
namespace QuanLyKhachSan.Services {
  public class TraPhongService {
    ...
    public KetQuaXuLy LapPhieuDenBu(string soDB, string soDat, string phong, DateTime ngay, string maNV, List < DenBuItem > ds) {
      if (string.IsNullOrWhiteSpace(soDB) || string.IsNullOrWhiteSpace(soDat) || string.IsNullOrWh iteSpace(phong) || string.IsNullOrWhiteSpace(maNV) || ds == null || ds.Count == 0) return KetQuaXuLy.Fail("Phiếu đền bù chưa đủ thông tin.");
    ...
    public KetQuaXuLy LapHoaDon(string soHD, string soDat, DateTime ngay, string maNV, int soNgay) {
      if (string.IsNullOrWhiteSpace(soHD) || string.IsNullOrWhiteSpace(soDat) || string.IsNull OrWhiteSpace(maNV) || soNgay <= 0) return KetQuaXuLy.Fail("Thông tin hóa đơn chưa hợp lệ.");
      try {
        ...
        Db.Execute("INSERT INTO HoaDon(SoHoaDon,SoPhieuDat,NgayLap,MaNV,SoNgayTinhTien,TienPhong,TienDichVu,TrangThai) VALUES(@h,@s,@n,@nv,@ng,@p,@d,N'Chưa thanh toán')", new SqlParameter("@h", soHD), new SqlParameter("@s", soDat), new SqlParameter("@n", ngay), new SqlParameter("@nv", maNV), new SqlParameter("@ng", soNgay), new SqlParameter("@p", phong), new SqlParameter("@d", dv));
    ...
    public KetQuaXuLy ThanhToan(string maTT, string soHD, DateTime ngay, string hinhThuc, decimal tien) {
      if (string.IsNullOrWhiteSpace(maTT) || string.IsNullOrWhiteSpace(soHD) || string.IsNullOr WhiteSpace(hinhThuc) || tien <= 0) return KetQuaXuLy.Fail("Thông tin thanh toán không hợp lệ.");
      using(var cn = Db.OpenConnection()) using(var tx = cn.BeginTransaction()) {
        try {
