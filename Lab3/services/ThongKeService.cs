...
CAST(NgayLap AS date) BETWEEN @tu AND @den) TongDenBu", new SqlParameter("@tu", tu.Date), new SqlParameter("@den", den.Date));
    }
...
GROUP BY d.MaDV,d.TenDV ORDER BY TongTien DESC", new SqlParameter("@tu", tu.Date), new SqlParameter("@den", den.Date));
    }
  }
}